...
Local bridge messages are `start`, `ready`, `transcript`, `speech`, `speech.result`, `video.start`, `video.stop`, `video.state`, `stop`, and `stopped`. The checked-in `InvitationBoundVoiceSession` supplies the fixed VAD, participant filtering, single-STT lifecycle, local TTS publication, barge-in, and idempotent-stop core for a Hermes Desktop bridge implementation.

For virtual video, the bridge must rasterize only the supplied Woka textures or configured asset reference, publish it as the LiveKit camera track, and acknowledge `publishing` only after the track is live. `VirtualCameraRenderer` provides frame pacing, speaking animation phases, exact RGBA size validation, and a rolling bitrate gate. The platform currently caps agent video at 640×360, 15 FPS, and 600 kbit/s. A bridge disconnect must unpublish all local tracks before closing so the platform can safely mark camera and microphone state false.
